...
const (
	...
	secretKey     = "token"
	dirPath       = "/srv/cloudprovider/aws/webtoken"
)
...
func (m *mutator) Mutate(ctx context.Context, new, old client.Object) error {
	...
	var (
		...
		secretKey = kutil.Key(new.GetNamespace(), secretName)
	)
	...
	if err := ensureVolume(terraformerPod); err != nil {
		return err
	}

	if err := ensureVolumeMount(terraformerPod, idx, dirPath); err != nil {
		return err
	}

	return ensureEnvVar(terraformerPod, idx, dirPath+"/token")
}
...
func ensureVolume(p *corev1.Pod) error {
...
func ensureEnvVar(p *corev1.Pod, idx int, tokenPath string) error {
	p.Spec.Containers[idx].Env = append(
		p.Spec.Containers[idx].Env,
		corev1.EnvVar{
